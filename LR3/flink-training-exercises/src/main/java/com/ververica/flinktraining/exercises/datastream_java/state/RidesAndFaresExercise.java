...
public class RidesAndFaresExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends RichCoFlatMapFunction<TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		public void open(Configuration config) throws Exception {
			this.rideState = getRuntimeContext().getState(new ValueStateDescriptor<>("lastRide", TaxiRide.class));
			this.fareState = getRuntimeContext().getState(new ValueStateDescriptor<>("lastFare", TaxiFare.class));
		}

		@Override
		public void flatMap1(TaxiRide ride, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			TaxiFare fare = fareState.value();  // Ищем fare для этой ride
			if (fare != null) {
				fareState.clear();  // Очищаем state
				out.collect(new Tuple2<>(ride, fare));
			} else {
				rideState.update(ride);  // Ждём fare, сохраняем ride
			}
		}

		@Override
		public void flatMap2(TaxiFare fare, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			TaxiRide ride = rideState.value();  // Ищем ride для этой fare
			if (ride != null) {
				rideState.clear();  // Очищаем state
				out.collect(new Tuple2<>(ride, fare));
			} else {
				fareState.update(fare);  // Ждём ride, сохраняем fare
			}
		}
	}
}